...
class Add extends \CarlBennett\Tools\Controllers\Base
{
  ...
  public function invoke(?array $args): bool
  {
    ...
    if (!$this->model->active_user || !$this->model->active_user->getAclObject()->getAcl(\CarlBennett\Tools\Libraries\User\Acl::ACL_PLEX_USERS))
    {
      $this->model->_responseCode = 401;
      return true;
    }

    $this->model->plex_user = new PlexUser(null);
  ...
  protected function post(HTTPForm $form): string
  {
    ...
    if (empty($this->model->plex_username) && empty($this->model->plex_email))
